...
@section('page-title', 'Chỉnh Sửa Màu Sắc')
@section('page-subtitle', 'Chỉnh sửa thông tin màu sắc sản phẩm')

@section('content')
<div style="max-width: 600px; margin: 0 auto;">
    <form method="POST" action="{{ route('admin.colors.update', $color) }}" style="display: flex; flex-direction: column; gap: var(--sp-2xl);">
        @csrf
        @method('PUT')
        ...
        <div class="panel" style="padding: var(--sp-xl);">
            <div style="display: flex; flex-direction: column; gap: var(--sp-lg);">
                <div>
                    ...
                    <input type="text" id="name" name="name" value="{{ old('name', $color->name) }}" required placeholder="Ví dụ: Trắng">
                ...
                <div>
                    ...
                    <input type="text" id="hex_code" name="hex_code" value="{{ old('hex_code', $color->hex_code) }}" required placeholder="#000000">
                ...
                <label style="display: flex; align-items: center; gap: var(--sp-lg); cursor: pointer; padding: var(--sp-lg); background: linear-gradient(90deg, rgba(0,212,255,0.05), transparent); border-radius: var(--radius-md); border: var(--border-thin);">
                    ...
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', $color->is_active) ? 'checked' : '' }} style="width: 18px; height: 18px; cursor: pointer;">
                    <div>
        ...
        <div style="display: flex; gap: var(--sp-lg); justify-content: flex-end;">
            ...
            <button type="submit" class="btn btn-primary"><i class="fas fa-save"></i> <span>Cập nhật</span></button>
        </div>
    </form>
</div>
@endsection
