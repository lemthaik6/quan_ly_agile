                        ...
                        @foreach($colors as $color)
                            <div style="display: grid; grid-template-columns: 1fr 120px; gap: 10px; align-items: center;">
                                <label style="display: flex; align-items: center; gap: 12px;">
                                    <input type="checkbox" name="product_colors[ids][]" value="{{ $color->id }}" {{ in_array($color->id, old('product_colors.ids', $product->colors->pluck('id')->toArray())) ? 'checked' : '' }}>
                                ...
                                <input type="number" name="product_colors[stock][{{ $color->id }}]" value="{{ old('product_colors.stock.' . $color->id, $product->colors->firstWhere('id', $color->id)->stock_quantity ?? 0) }}" min="0" placeholder="Kho">
                            </div>
                        @endforeach
                    </div>
                @else
                ...
                @if($sizes->count() > 0)
                    <div style="display: grid; gap: 12px;">
                        @foreach($sizes as $size)
                            <div style="display: grid; grid-template-columns: 1fr 120px; gap: 10px; align-items: center;">
                                <label style="display: flex; align-items: center; gap: 12px;">
                                    <input type="checkbox" name="product_sizes[ids][]" value="{{ $size->id }}" {{ in_array($size->id, old('product_sizes.ids', $product->sizes->pluck('id')->toArray())) ? 'checked' : '' }}>
                                    {{ $size->name }}
                                </label>
                                <input type="number" name="product_sizes[stock][{{ $size->id }}]" value="{{ old('product_sizes.stock.' . $size->id, $product->sizes->firstWhere('id', $size->id)->stock_quantity ?? 0) }}" min="0" placeholder="Kho">
                            </div>
                        @endforeach
                    </div>
                @else
