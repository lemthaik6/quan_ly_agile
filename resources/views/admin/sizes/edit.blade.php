...
@section('page-title', 'Chỉnh Sửa Kích Cỡ')
@section('page-subtitle', 'Chỉnh sửa thông tin kích cỡ sản phẩm')

@section('content')
<div style="max-width: 600px; margin: 0 auto;">
    <form method="POST" action="{{ route('admin.sizes.update', $size) }}" style="display: flex; flex-direction: column; gap: var(--sp-2xl);">
        @csrf
        @method('PUT')
        ...
        <div class="panel" style="padding: var(--sp-xl);">
            <div style="display: flex; flex-direction: column; gap: var(--sp-lg);">
                <div>
                    ...
                    <input type="text" id="name" name="name" value="{{ old('name', $size->name) }}" required placeholder="Ví dụ: M">
                ...
                <label style="display: flex; align-items: center; gap: var(--sp-lg); cursor: pointer; padding: var(--sp-lg); background: linear-gradient(90deg, rgba(0,212,255,0.05), transparent); border-radius: var(--radius-md); border: var(--border-thin);">
                    ...
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', $size->is_active) ? 'checked' : '' }} style="width: 18px; height: 18px; cursor: pointer;">
                    <div>
